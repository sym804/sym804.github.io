...
const SEV_PRI = [
  { sev: 'Critical', pri: 'High', action: '즉시 hotfix' },
  { sev: 'Critical', pri: 'Low', action: '다음 릴리즈에 우선' },
  { sev: 'Minor', pri: 'High', action: '스프린트 내 처리' },
  { sev: 'Minor', pri: 'Low', action: '백로그 / 추후' },
];
...
export function DefectManagementSection() {
  return (
    <SectionShell
      ...
      why={
        <p>
          결함은 자산이다. 제대로 분류하고 라이프사이클을 통해 처리할 때만 학습이 누적된다. 비난 없이
          사실을 정리하고, 재발 방지로 끝맺을 때 결함이 다음 결함을 줄인다.
        </p>
      }
      body={
        <>
          ...
          <div>
            ...
            <p className="text-muted-foreground mb-3">
              Severity 는 객관적 영향도, Priority 는 처리 시급성이다. 둘은 곱의 관계가 아니라 별개의
              차원이며, 조합에 따라 처리 시점이 달라진다.
            </p>
          ...
          <div>
            ...
            <ul className="list-disc list-inside space-y-1 text-muted-foreground">
              <li>비난 문화: 사람을 원인으로 지목하면 사실이 숨겨진다.</li>
              <li>RCA 의 형식화: 템플릿만 채우고 액션이 사라지면 학습이 누적되지 않는다.</li>
              <li>회고 액션의 미실행: 다음 회고에서 같은 액션이 반복된다.</li>
            </ul>
          </div>
        </>
      }
      triangleRecap={
        <TriangleRecap
          notes={{
            risk: 'RCA 결과는 리스크 매트릭스의 갱신 인풋이 된다.',
            'shift-left': '라이프사이클 단축의 핵심은 인지 단계를 앞당기는 것이다.',
            roi: '회고 / RCA 의 누적 학습이 장기 자동화 ROI 의 기반이 된다.',
          }}
        />
      }
    />
