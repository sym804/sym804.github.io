...
export function ShiftLeftSection() {
  return (
    <SectionShell
      ...
      why={
        <p>
          결함은 늦게 발견될수록 비쌉니다. QA 가 요구사항부터 개입하면 잘못된 가정 / 모호한 수용 기준 /
          누락된 비기능 요구가 코드 작성 전에 잡힙니다. 단계마다 게이트를 두면 결함이 다음 단계로
          누수되는 경로 자체가 좁아집니다.
        </p>
      }
      body={
        <>
          ...
          <div>
            ...
            <ul className="list-disc list-inside space-y-1 text-muted-foreground">
              <li>배포 직전 일괄 테스트: 결함 발견 시점이 가장 늦은 곳에 몰려 비용이 최대화됩니다.</li>
              <li>게이트 우회 (no-verify) 의 일상화: 한 번의 우회가 다음 우회를 부릅니다.</li>
              <li>게이트 통과율 KPI 의 의례화: 100% 통과만 추구하면 게이트가 수축됩니다.</li>
            </ul>
          </div>
        </>
      }
      fieldNote={
        <FieldNote assets={(META.caseAssets ?? []).map((c) => ({ title: c.title, href: `../${c.asset}.html` }))}>
          <p>
            ...
            에이전트 자동 전환) 는 신호 기반 분기로 처리합니다. AI 특유의 비결정성을 단계 분리로 흡수한
            Shift-Left 구조.
          </p>
        </FieldNote>
      }
      triangleRecap={
        <TriangleRecap
          notes={{
            risk: '위험이 큰 단계일수록 게이트의 강도와 차단 정책이 강해집니다.',
            'shift-left': '본 섹션 자체가 Shift-Left 의 본진입니다.',
            roi: '게이트 자동화의 ROI 는 단계가 왼쪽일수록 높습니다.',
          }}
        />
      }
    />
