...
export function TestStrategySection() {
  return (
    <SectionShell
      ...
      why={
        <p>
          어디에서 무엇을 테스트할지가 비용과 신뢰도를 동시에 결정한다. 같은 결함을 어느 레벨에서
          잡느냐에 따라 비용이 큰 폭으로 벌어진다. 전략 없이 자동화부터 늘리면 아이스크림 콘 함정에
          빠진다.
        </p>
      }
      body={
        <>
          <div>
            ...
            <p className="text-muted-foreground mb-4">
              피라미드 비율은 도메인 / 안정성 / 변경 빈도에 따라 조정한다. 일반 가이드라인 (Unit 70 /
              Integration 20 / E2E 10) 은 출발선일 뿐 종착선이 아니다.
            </p>
            <TestPyramid />
          </div>
          <div>
      ...
      fieldNote={
        <FieldNote assetTitle={META.caseAssetTitle ?? 'YM TestCase'} assetHref={`../${META.caseAsset}.html`}>
          <p>
            ...
            템플릿화된 테스트 케이스가 누적되면서 회귀 자동화 후보 선정도 데이터 기반으로 가능해진다.
          </p>
        </FieldNote>
      }
      triangleRecap={
        <TriangleRecap
          notes={{
            risk: '레벨 × 타입 매트릭스의 셀 우선순위는 리스크 점수가 정한다.',
            'shift-left': '단위 레벨 비중을 키우는 것은 결함을 가장 이른 시점에 잡는 것이다.',
            roi: '회귀 / 접근성 / 성능 자동화는 장기 ROI 가 가장 큰 영역이다.',
          }}
        />
      }
    />
